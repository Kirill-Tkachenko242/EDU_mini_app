/*
  # Setup grades and schedule system

  1. New Tables
    - grades: Stores student grades
    - grade_history: Tracks grade changes
    - schedule_events: Manages class schedule
    - schedule_conflicts: Tracks scheduling conflicts
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Teachers can manage all grades" ON grades;
  DROP POLICY IF EXISTS "Students can view their own grade history" ON grade_history;
  DROP POLICY IF EXISTS "Teachers can insert grade history" ON grade_history;
  DROP POLICY IF EXISTS "Teachers can view all grade history" ON grade_history;
  DROP POLICY IF EXISTS "Anyone can view schedule events" ON schedule_events;
  DROP POLICY IF EXISTS "Teachers can manage their own schedule events" ON schedule_events;
  DROP POLICY IF EXISTS "Teachers can view schedule conflicts" ON schedule_conflicts;

  -- Drop triggers
  DROP TRIGGER IF EXISTS grades_updated_at ON grades;
  DROP TRIGGER IF EXISTS schedule_events_updated_at ON schedule_events;
  DROP TRIGGER IF EXISTS schedule_conflicts_check ON schedule_events;

  -- Drop functions
  DROP FUNCTION IF EXISTS handle_grades_updated_at();
  DROP FUNCTION IF EXISTS handle_schedule_events_updated_at();
