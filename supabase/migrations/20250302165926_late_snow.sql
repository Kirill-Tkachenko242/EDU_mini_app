...
-- Check if department column exists in professors table
DO $$
BEGIN
  ...
  IF EXISTS (
    ...
    -- Make department column nullable if it's not already
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM professors WHERE "fullName" = 'Иванов Иван Иванович') THEN
    INSERT INTO professors ("fullName", "phoneNumber", email, position, faculty_id, department)
    VALUES ('Иванов Иван Иванович', '+7 (900) 123-45-67', 'ivanov@university.edu', 'Профессор', it_faculty_id, 'Кафедра информационных технологий');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM professors WHERE "fullName" = 'Петрова Мария Сергеевна') THEN
    INSERT INTO professors ("fullName", "phoneNumber", email, position, faculty_id, department)
    VALUES ('Петрова Мария Сергеевна', '+7 (900) 765-43-21', 'petrova@university.edu', 'Доцент', econ_faculty_id, 'Кафедра экономики');
  END IF;
END$$;